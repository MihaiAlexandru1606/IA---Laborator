...
def testL(Lv, Lc, env, idx):
    ...
    if not isinstance(Lv, list):
        ...
        Lc = "a list of length " + str(len(Lc))
    else:
        "a list also containing " + ", ".join([e for e in Lc if e not in Lv])[:-2]
...
testBatch[4] = [(testResolves, [
    ...
    ("make_or(make_atom('P', make_const(5)), make_atom('Q', make_function_call(add, make_var('y'), make_var('x'))))",
     "make_or(make_neg(make_atom('P', make_function_call(add, make_var('x')))), make_neg(make_atom('Q', make_var('x'))))",
     False)
])]
